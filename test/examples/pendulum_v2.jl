...
obj = [[ct for t = 1:horizon-1]..., cT]

# ## constraints 
eq1 = Constraint((x, u, w) -> x - x1, num_state, num_action)
eqT = Constraint((x, u, w) -> x - xT, num_state, 0)
eq = [eq1, [Constraint() for t = 2:horizon-1]..., eqT]

ineq = [Constraint() for t = 1:horizon]

soc = [[Constraint()] for t = 1:horizon]
...
u_guess = [1.0 * randn(num_action) for t = 1:horizon-1]
