@testset "Solver problem: Quadratic program w/ inequality constraints" begin
    ...
    @test all(solver.variables[solver.indices.variables] .> -1.0e-4)
    ...
    rz = [
            ...
            Diagonal(solver.variables[solver.indices.cone_slack_dual]) zeros(num_cone, num_equality) Diagonal(solver.variables[solver.indices.cone_slack])
    ]
