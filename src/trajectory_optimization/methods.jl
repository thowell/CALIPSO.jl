...
function ProblemMethods(trajopt::TrajectoryOptimizationProblem) 
    ProblemMethods(
        ...
        zeros(length(trajopt.sparsity.objective_jacobian_variables_variables)), 
        zeros(length(trajopt.sparsity.objective_jacobian_variables_parameters)), 
        ...
        zeros(length(trajopt.sparsity.dynamics_jacobian_variables) + length(trajopt.sparsity.equality_jacobian_variables)), 
        zeros(length(trajopt.sparsity.dynamics_jacobian_parameters) + length(trajopt.sparsity.equality_jacobian_parameters)),
        vcat(trajopt.sparsity.dynamics_jacobian_variables..., trajopt.sparsity.equality_jacobian_variables...), 
        vcat(trajopt.sparsity.dynamics_jacobian_parameters..., trajopt.sparsity.equality_jacobian_parameters...),
        (f, z, θ, y) -> nothing,
        ...
        zeros(length(trajopt.sparsity.dynamics_jacobian_variables_variables) + length(trajopt.sparsity.equality_jacobian_variables_variables)), 
        zeros(length(trajopt.sparsity.dynamics_jacobian_variables_parameters) + length(trajopt.sparsity.equality_jacobian_variables_parameters)),
        vcat(trajopt.sparsity.dynamics_jacobian_variables_variables..., trajopt.sparsity.equality_jacobian_variables_variables...), 
        vcat(trajopt.sparsity.dynamics_jacobian_variables_parameters..., trajopt.sparsity.equality_jacobian_variables_parameters...), 
        (c, z, θ) -> cone!(c, trajopt, z, θ),
        (j, z, θ) -> cone_jacobian_variables!(j, trajopt, z, θ),
        (j, z, θ) -> cone_jacobian_parameters!(j, trajopt, z, θ),
        zeros(length(trajopt.sparsity.nonnegative_jacobian_variables) + length(trajopt.sparsity.second_order_jacobian_variables)), 
        zeros(length(trajopt.sparsity.nonnegative_jacobian_parameters) + length(trajopt.sparsity.second_order_jacobian_parameters)),
        ...
        zeros(length(trajopt.sparsity.nonnegative_jacobian_variables_variables) + length(trajopt.sparsity.second_order_jacobian_variables_variables)), 
        zeros(length(trajopt.sparsity.nonnegative_jacobian_variables_parameters) + length(trajopt.sparsity.second_order_jacobian_variables_parameters)),
